...
def email():
    ...
    if request.method == 'POST':
        ...
        try:
            for index, row in df.iterrows():
                # Re-extract data or use data from above loop if structured differently
                student_name = str(row.get('Student Name', ''))
                # *** IMPORTANT: Use ACTUAL emails here, not the placeholder ***
                student_email = str(row.get('Student Email', '')) # Get actual student email from CSV
                tutor_name = str(row.get('Tutor Name', ''))
                tutor_email = str(row.get('Tutor Email', ''))     # Get actual tutor email from CSV

                # Check if emails are valid before proceeding (basic check)
                if not student_email or '@' not in student_email:
                     print(f"Skipping row {index}: Invalid or missing student email.")
                     continue
                if not tutor_email or '@' not in tutor_email:
                     print(f"Skipping row {index}: Invalid or missing tutor email.")
                     continue

                student_grade = str(row.get('Student Grade', ''))
                tutor_grade = str(row.get('Tutor Grade', ''))
                time_slot = str(row.get('Time', 'N/A'))
                subject = str(row.get('Student Courses', 'N/A'))
                ...
                # Use dummy/placeholder emails ONLY IF actual emails are missing AND you intend to send to a test address
                # It's better practice to ensure the CSV has correct emails.
                # student_email_actual = student_email if student_email and '@' in student_email else "hliao38@geffenacademy.ucla.edu" # Your test email
                # tutor_email_actual = tutor_email if tutor_email and '@' in tutor_email else "hliao38@geffenacademy.ucla.edu" # Your test email
                ...
                # Send Student Email if needed
                if not student_email_status:
                    subject_student = f'Peer Tutoring Schedule'
                    message_student = (f'Dear {student.name or "Student"},\n\n'
                                       f'You have been matched with {tutor.name or "a tutor"} for these classes: {subject or "specified subjects"}. '
                                       f'{tutor.name or "Your tutor"} is available to meet with you at {time_slot or "the scheduled time"}.\n\n'
                                       f'Regards,\n'
                                       f'Geffen Peer Tutoring Team')
                    # *** Use the actual student object with the correct email ***
                    auto_email(student, subject_student, message_student)
                    df.loc[index, 'Student Email Status'] = True # Use .loc for safer assignment
                    send_count += 1

                # Send Tutor Email if needed
                if not tutor_email_status:
                    subject_tutor = f'Peer Tutoring Schedule'
                    # Use the same conditional logic as the preview
                    if info and "nan" not in info.lower():
                         message_tutor = (f'Dear {tutor.name or "Tutor"},\n\n'
                                         f'You have been matched with {student.name or "a student"} for these classes: {subject or "specified subjects"}. '
                                         f'{student.name or "The student"} is available to meet with you at {time_slot or "the scheduled time"}.\n\n'
                                         f'Student Comments: {info}\n\n'
                                         f'Regards,\n'
                                         f'Geffen Peer Tutoring Team')
                    else:
                         message_tutor = (f'Dear {tutor.name or "Tutor"},\n\n'
                                         ...
                                         f'Regards,\n'
                                         f'Geffen Peer Tutoring Team')
                    # *** Use the actual tutor object with the correct email ***
                    auto_email(tutor, subject_tutor, message_tutor)
                    df.loc[index, 'Tutor Email Status'] = True # Use .loc for safer assignment
                    send_count += 1

            # --- Sending emails to unmatched students ---
            # This part seems separate, ensure 'not_matched_students' is populated correctly elsewhere
            # Also ensure these student objects have valid emails
            # for student_obj, details in not_matched_students.items():
            #     subject_unmatched = 'Peer Tutoring Arrangement'
            #     # Ensure email is set correctly, maybe read from original data?
            #     # student_obj.email = "hliao38@geffenacademy.ucla.edu" # Using test email? Be careful.
            #     reason = details[0]
            #     suggestions = details[1]
            #     if not suggestions:
            #         message_unmatched = (f'Dear {student_obj.name},\n\n'
            ...
            #         message_unmatched = (f'Dear {student_obj.name},\n\n'
            #                            f'Unfortunately, we have not been able to match you with a tutor because {reason}.\n\n'
            #                            f'Here are possible time slots you could consider: {suggestions}\n\n'
            #                            f'Regards,\n'
            #                            f'Geffen Peer Tutoring Team')
            #     # auto_email(student_obj, subject_unmatched, message_unmatched)
            #     # send_count += 1 # Increment if sending these too
